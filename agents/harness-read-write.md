...
## 절차 — mode=create
1. 전달받은 템플릿 스키마(≤50줄)에 맞춰 각 문서를 채워 지정 경로(`docs/*.md`, `{module}/CLAUDE.md`)에 **Write** 한다.

## 절차 — mode=update
1. 기존 문서를 먼저 Read 한다. **기존 내용은 보존**하고, 코드 실상과 어긋난 부분만 **Edit 로 최소 diff** 갱신한다. 전체 재작성·갈아엎기 금지.
2. module CLAUDE.md 는 갱신 후에도 ≤50줄을 유지한다.
...
- 워크플로우가 `schema` 를 지정해 호출하면, 자유 텍스트 대신 그 schema(작성 경로 / `{TBD}` 목록 / 변경 요약)에 맞춰 **구조화 반환**한다.
