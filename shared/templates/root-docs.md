...
- 진행 추적 파일 `.harness/run-*.md` 참조 규칙 / 완료 기준 확인 규칙
...
- 통합/E2E 작성 규칙:
  ...
  - 레벨별 실행 환경: 통합=JVM(Robolectric/in-memory) / E2E=기기(Espresso/Maestro)
- 검증 명령어 (실행 가능한 명령 — work/bug-fix 루프의 입력원):
  - build / lint / Unit
  - Integration(JVM) — work 자동 루프에 포함
  - E2E(기기) — 사람 게이트, 수동 트리거
